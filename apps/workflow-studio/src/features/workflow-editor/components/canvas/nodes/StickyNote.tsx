...
function StickyNote({ id, data, selected }: NodeProps<StickyNoteData>) {
  ...
  return (
    <div
      className={`
        relative min-w-[200px] min-h-[150px] max-w-[300px] rounded-lg border-2 shadow-md
    ...
    >
      ...
      {isEditing ? (
        <textarea
          ...
          className={`
            nodrag w-full h-full min-h-[150px] p-3 bg-transparent resize-none
        ...
        <div
          ...
          className={`
            w-full h-full min-h-[150px] p-3 cursor-text whitespace-pre-wrap
            ${colors.text}
            ${!content ? 'opacity-50' : ''}
          `}
        >
